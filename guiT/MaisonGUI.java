	...
	private void init() {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (key == 'P' || key == 'R') {
					manager.getFather().supAllAction();
					...
					if(key =='O' || key == 'E') {
						manager.getFather().supAllAction();
						manager.getFather().setActionAnimal(true);
						manager.getFather().setTarget(manager.getCat());
						manager.getFather().setPunishment(key == 'O'); // 'O' pour punition, 'E' pour récompense
						System.out.println("Father cible le chat avec " + (key == 'O' ? "punition" : "récompense"));
					}
				}
			}
		});
